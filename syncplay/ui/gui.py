...
from PyQt5.QtCore import  QSettings, QSize, QPoint, QUrl, QLine, Qt
...
class UserlistItemDelegate(QtWidgets.QStyledItemDelegate):
    ...
    def paint(self, itemQPainter, optionQStyleOptionViewItem, indexQModelIndex):
        ...
        midY = int(old_div((optionQStyleOptionViewItem.rect.y() + optionQStyleOptionViewItem.rect.bottomLeft().y()), 2))
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def openAddMediaDirectoryDialog(self, MediaDirectoriesTextbox, MediaDirectoriesDialog):
        ...
        folderName = str(QtGui.QFileDialog.getExistingDirectory(self,None,self.getInitialMediaDirectory(includeUserSpecifiedDirectories=False),options))
        if folderName:
    ...
    def identifyAsController(self):
        ...
        controlpassword, ok = QtGui.QInputDialog.getText(self, msgboxtitle, msgboxtext, QtWidgets.QLineEdit.Normal, "")
    ...
    def checkForUpdates(self, userInitiated=False):
        ...
        if updateURL is not None:
            reply = QtWidgets.QMessageBox.question(self, "Syncplay",
                                        updateMessage, QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No)
